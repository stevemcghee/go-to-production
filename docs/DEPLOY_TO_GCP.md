...
# enable_slos = true # Only enable AFTER the app is deployed!
EOF
...
## 9. Enable SLOs and Alerting

Service Level Objectives (SLOs) rely on Prometheus metrics generated by the application. Therefore, you must deploy the application and generate some traffic before Terraform can create the SLO resources.

Once your app is running:
1. Update `terraform/terraform.tfvars` and set `enable_slos = true`.
2. Generate some traffic to your app (e.g., by visiting it in your browser).
3. Run `terraform apply` in the `terraform/` directory.